...
{
    TEST_CASE("create symlink")
    {
        vfs::create_symlink task;

        SUBCASE("basic")
        {
            task.target("/existing/file.txt").name("/new/hardlink.txt").compile();

            CHECK_EQ(*task.dump(), "ln -s  \"/existing/file.txt\" \"/new/hardlink.txt\"");
        }
        ...
        SUBCASE("basic force")
        {
            ...
            CHECK_EQ(*task.dump(), "ln -s --force \"/existing/file.txt\" \"/new/hardlink.txt\"");
        }
