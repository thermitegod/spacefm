...
vfs::create_file&
vfs::create_file::path(const std::filesystem::path& path) noexcept
{
    if (path.empty())
    {
...
void
vfs::create_file::compile() noexcept
{
    if (this->ec_)
    {
        return;
    ...
    this->cmd_ = std::format("touch {}", vfs::execute::quote(this->path_.string()));
}
