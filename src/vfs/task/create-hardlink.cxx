...
vfs::create_hardlink&
vfs::create_hardlink::force() noexcept
{
    this->options_ += "--force ";
    return *this;
}
...
vfs::create_hardlink&
vfs::create_hardlink::target(const std::filesystem::path& path) noexcept
{
    if (path.empty())
    {
...
vfs::create_hardlink&
vfs::create_hardlink::name(const std::filesystem::path& path) noexcept
{
    if (path.empty())
    {
        this->ec_ = vfs::error_code::task_empty_path;
...
void
vfs::create_hardlink::compile() noexcept
{
    if (this->ec_)
    {
        return;
