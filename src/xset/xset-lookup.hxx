...
namespace xset
{
    class invalid_name : virtual public std::exception
    {
      protected:
        std::string error_message;

      public:
        explicit invalid_name(const std::string_view msg) : error_message(msg){};
        virtual ~invalid_name() throw(){};
    ...
    enum class name
    {
        ...
        // also the only xset::name not implemented in xset_name_map
        // trying to lookup a XSet name for this will throw xset::invalid_name
        custom,
    ...
    const std::string get_name_from_xsetname(xset::name name);

    xset::name get_xsetname_from_panel(panel_t panel, xset::panel panel_var);
    const std::string get_name_from_panel(panel_t panel, xset::panel name);

    xset::name get_xsetname_from_panel_mode(panel_t panel, xset::panel name,
                                            xset::main_window_panel mode);
    const std::string get_name_from_panel_mode(panel_t panel, xset::panel name,
                                               xset::main_window_panel mode);

    xset::var get_xsetvar_from_name(const std::string_view name);
    const std::string get_name_from_xsetvar(xset::var name);

    // main window panel mode
    const std::string get_window_panel_mode(xset::main_window_panel mode);
} // namespace xset
