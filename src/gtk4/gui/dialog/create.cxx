...
gui::dialog::create::create(Gtk::ApplicationWindow& parent, const std::filesystem::path& cwd,
                            const std::shared_ptr<vfs::file>& file,
                            const gui::dialog::create_mode mode,
                            const std::shared_ptr<config::settings>& settings)
    : settings_(settings), file_(file), mode_(mode)
{
    ...
    if (mode_ == create_mode::link && file_)
    {
        const auto full_name = file_->name();
        full_path_ = cwd / full_name;
        new_path_ = full_path_;
        is_dir_ = file_->is_directory(); // is_dir is dynamic for create
        is_link_ = file_->is_symlink();
    }
    else
    {
        full_path_ = vfs::utils::unique_path(cwd, "new");
        new_path_ = full_path_;
        is_dir_ = false; // is_dir is dynamic for create
        is_link_ = false;
    }
    ...
    if (!new_path_.empty() && file_)
    {
        entry_target_.set_text(new_path_.c_str());
    }

    { // Target Signals
        auto s = entry_target_.signal_changed().connect(
            sigc::bind(sigc::mem_fun(*this, &create::on_move_change), buf_full_path_));
    ...
    { // Filename Signals
        ...
        key_controller->signal_key_pressed().connect(sigc::mem_fun(*this, &create::on_key_press),
                                                     false);

        input_full_name_.add_controller(key_controller);

        auto s = buf_full_name_->signal_changed().connect(
            sigc::bind(sigc::mem_fun(*this, &create::on_move_change), buf_full_name_));
        on_move_change_signals_.push_back(s);
    }
    ...
    { // Parent Signals
        ...
        auto s = buf_path_->signal_changed().connect(
            sigc::bind(sigc::mem_fun(*this, &create::on_move_change), buf_path_));
        on_move_change_signals_.push_back(s);
    }
    ...
    action_filename_->signal_activate().connect(
        ...
        {
            settings_->dialog.create.filename = !settings_->dialog.create.filename;
            on_toggled();
        });
    ...
    action_path_->signal_activate().connect(
        [this](const auto& /*value*/)
        {
            settings_->dialog.create.path = !settings_->dialog.create.path;
            on_toggled();
        });
    ...
    action_parent_->signal_activate().connect(
        [this](const auto& /*value*/)
        {
            settings_->dialog.create.parent = !settings_->dialog.create.parent;
            on_toggled();
        });
    context_action_group_->add_action(action_parent_);

    action_confirm_ = Gio::SimpleAction::create_bool("confirm", settings_->dialog.create.confirm);
    action_confirm_->signal_activate().connect(
        [this](const auto& /*value*/)
        {
            settings_->dialog.create.confirm = !settings_->dialog.create.confirm;
            action_confirm_->set_state(
                Glib::Variant<bool>::create(settings_->dialog.create.confirm));
        });
...
bool
gui::dialog::create::on_key_press(std::uint32_t keyval, std::uint32_t keycode,
...
void
gui::dialog::create::on_button_ok_clicked()
{
    ...
    if (!std::filesystem::exists(path))
    {
        ...
        if (!settings_->dialog.create.confirm)
        {
    ...
    if (new_link)
    { // new link task
        std::string entry_text = entry_target_.get_text();
...
void
gui::dialog::create::on_button_cancel_clicked()
{
    close();
}

void
gui::dialog::create::on_button_revert_clicked()
{
...
void
gui::dialog::create::on_button_options_clicked()
{
    context_menu_.popup();
}

void
gui::dialog::create::on_button_open_clicked()
{
    auto_open_ = true;
    on_button_ok_clicked();
}
...
void
gui::dialog::create::on_move_change(Glib::RefPtr<Gtk::TextBuffer>& widget)
{
...
void
gui::dialog::create::select_input() noexcept
{
...
gui::dialog::create::on_opt_toggled()
{
    ...
    // title
    set_title(std::format("Create New {}", desc));
...
void
gui::dialog::create::on_toggled()
{
    bool someone_is_visible = false;

    // entries
    if (settings_->dialog.create.filename)
    {
    ...
    if (settings_->dialog.create.parent)
    {
        someone_is_visible = true;
    ...
    if (settings_->dialog.create.path)
    {
        someone_is_visible = true;
    ...
    {
        settings_->dialog.create.filename = true;
        on_toggled();
    }
}
