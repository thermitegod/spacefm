...
static void on_popup_show_hidden(GtkMenuItem* menuitem, PtkFileBrowser* browser)
{
    if (browser)
...
static void on_copycmd(GtkMenuItem* menuitem, PtkFileMenu* data, XSet* set2)
{
    XSet* set;
    if (menuitem)
...
static void on_popup_rootcmd_activate(GtkMenuItem* menuitem, PtkFileMenu* data, XSet* set2)
{
    XSet* set;
    if (menuitem)
        set = (XSet*)g_object_get_data(G_OBJECT(menuitem), "set");
...
static void on_popup_select_pattern(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    if (data->browser)
        ptk_file_browser_select_pattern(NULL, data->browser, NULL);
}

static void on_open_in_tab(GtkMenuItem* menuitem, PtkFileMenu* data)
{
...
static void on_open_in_panel(GtkMenuItem* menuitem, PtkFileMenu* data)
{
...
static void on_file_edit(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    xset_edit(data->browser, data->file_path, FALSE, TRUE);
}

static void on_file_root_edit(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    xset_edit(data->browser, data->file_path, TRUE, FALSE);
}

static void on_popup_sort_extra(GtkMenuItem* menuitem, PtkFileBrowser* file_browser, XSet* set2)
{
    XSet* set;
    if (menuitem)
        set = (XSet*)g_object_get_data(G_OBJECT(menuitem), "set");
...
static void on_popup_detailed_column(GtkMenuItem* menuitem, PtkFileBrowser* file_browser)
{
...
static void on_popup_toggle_view(GtkMenuItem* menuitem, PtkFileBrowser* file_browser)
{
    // get visiblity for correct mode
    FMMainWindow* main_window = (FMMainWindow*)file_browser->main_window;
    int p = file_browser->mypanel;
...
static void on_archive_show_config(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    ptk_handler_show_config(HANDLER_MODE_ARC, data->browser, NULL);
}

static void on_hide_file(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    if (data->browser)
        ptk_file_browser_hide_selected(data->browser, data->sel_files, data->cwd);
}

static void on_permission(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    if (data->browser)
                                       ...
                                       GtkAccelGroup* accel_group)
{
    ...
    set = xset_set_cb_panel(p, "show_devmon", on_popup_toggle_view, browser);
    set->b = xset_get_panel_mode(p, "show_devmon", mode)->b;
    if (set->b == XSET_B_TRUE)
        show_side = TRUE;
    set = xset_set_cb_panel(p, "show_dirtree", on_popup_toggle_view, browser);
    set->b = xset_get_panel_mode(p, "show_dirtree", mode)->b;
    if (set->b == XSET_B_TRUE)
        show_side = TRUE;
    set = xset_set_cb_panel(p, "show_book", on_popup_toggle_view, browser);
    set->b = xset_get_panel_mode(p, "show_book", mode)->b;
    if (set->b == XSET_B_TRUE)
        show_side = TRUE;
    set = xset_set_cb_panel(p, "show_sidebar", on_popup_toggle_view, browser);
    set->b = xset_get_panel_mode(p, "show_sidebar", mode)->b;
    set->disable = !show_side;
    xset_set_cb_panel(p, "show_hidden", on_popup_show_hidden, browser);
    ...
    set = xset_set_cb("view_thumb", main_window_toggle_thumbnails_all_windows, NULL);
    ...
    {
        set = xset_set_cb_panel(p, "list_large", on_popup_list_large, browser);
        set->disable = FALSE;
...
{ // either desktop or browser must be non-NULL
    ...
    if (sel_files)
    {
        ...
        if (handlers_slist)
        {
            ...
            set_arc_extract = xset_set_cb("arc_extract", on_popup_extract_here_activate, data);
            xset_set_ob1(set_arc_extract, "set", set_arc_extract);
            set_arc_extract->disable = no_write_access;

            set_arc_extractto = xset_set_cb("arc_extractto", on_popup_extract_to_activate, data);
            xset_set_ob1(set_arc_extractto, "set", set_arc_extractto);

            set_arc_list = xset_set_cb("arc_list", on_popup_extract_list_activate, data);
        ...
        if ((is_dir && browser) || (is_text && sel_files && !sel_files->next))
        {
            ...
            else if (browser && is_dir)
            {
                ...
                set = xset_set_cb("opentab_new", on_popup_open_in_new_tab_activate, data);
                for (i = 1; i < 11; i++)
                {
...
static void on_popup_open_activate(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    ...
    ptk_open_files_with_app(data->cwd, sel_files, NULL, data->browser, TRUE, FALSE);
...
static char* get_shared_desktop_file_location(const char* name)
{
    char* ret;
...
void app_job(GtkWidget* item, GtkWidget* app_item)
{
    ...
    switch (job)
    {
        case APP_JOB_DEFAULT:
            ...
            ptk_app_chooser_has_handler_warn(data->browser ? GTK_WIDGET(data->browser) : NULL,
                                             mime_type);
            break;
        case APP_JOB_REMOVE:
        ...
        case APP_JOB_EDIT:
            ...
            {
                char* share_desktop =
                    vfs_mime_type_locate_desktop_file(NULL, desktop_file->file_name);
        ...
        case APP_JOB_EDIT_LIST:
            ...
            if (!g_file_test(path, G_FILE_TEST_EXISTS))
            {
                ...
                g_free(path);
                path =
                    g_build_filename(g_get_user_data_dir(), "applications", "mimeapps.list", NULL);
            }
            xset_edit(data->browser, path, FALSE, TRUE);
            g_free(path);
            break;
...
static bool app_menu_keypress(GtkWidget* menu, GdkEventKey* event, PtkFileMenu* data)
{
    int job = -1;
...
static void on_app_menu_hide(GtkWidget* widget, GtkWidget* app_menu)
{
    gtk_widget_set_sensitive(widget, TRUE);
    gtk_menu_shell_deactivate(GTK_MENU_SHELL(app_menu));
}

static GtkWidget* app_menu_additem(GtkWidget* menu, char* label, char* stock_icon, int job,
                          ...
                          unsigned int button, uint32_t time)
{
    ...
    {
        path =
            g_build_filename(g_get_user_data_dir(), "applications", desktop_file->file_name, NULL);
        if (g_file_test(path, G_FILE_TEST_EXISTS))
        {
    ...
    g_signal_connect(app_menu, "selection-done", G_CALLBACK(gtk_widget_destroy), NULL);
    ...
    if (ao->path && GTK_IS_WIDGET(ao->file_browser) && g_file_test(ao->path, G_FILE_TEST_EXISTS))
    {
        ...
        if (ao->open_file)
        {
            ...
            else
            {
                ...
                ptk_open_files_with_app(cwd, sel_files, NULL, ao->file_browser, FALSE, TRUE);
                vfs_file_info_unref(file);
                g_list_free(sel_files);
            }
        }
