...
init_list_view(PtkFileBrowser* file_browser, GtkTreeView* list_view)
{
    ...
    int cols[] = {PTKFileListCol::COL_FILE_NAME,
    ...
    for (unsigned int i = 0; i < G_N_ELEMENTS(cols); i++)
    {
        ...
        else
        {
            ...
            if (width)
            {
                if (cols[j] == PTKFileListCol::COL_FILE_NAME && !app_settings.always_show_tabs &&
        ...
        if (cols[j] == PTKFileListCol::COL_FILE_NAME)
        {
            g_object_set(G_OBJECT(renderer),
                         /* "editable", true, */
                         "ellipsize",
        ...
        if (cols[j] == PTKFileListCol::COL_FILE_SIZE)
        ...
        !copy_dest && !move_dest)
    {
        ...
        if (path && std::filesystem::is_directory(path))
        {
            if (Glib::str_has_prefix(setname, "copy_loc"))
                ...
                move_dest = path;
            set2 = xset_get("copy_loc_last");
            xset_set_set(set2, XSetSetSet::XSET_SET_SET_DESC, path);
        }
        else
            return;
    }
