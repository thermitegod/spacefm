...
static void fill_combo_box(CustomElement* el, GList* arglist)
{
    ...
    if (set_default != -1)
    {
        ...
        gtk_entry_set_text(GTK_ENTRY(gtk_bin_get_child(GTK_BIN(combo))), default_value);
}
...
static char* get_tree_view_selected(CustomElement* el, const char* prefix)
{
    ...
    while (valid_iter)
    {
        ...
        if (gtk_tree_selection_iter_is_selected(tree_sel, &iter))
        {
            ...
            str = selected;
            selected =
                g_strdup_printf("%s%s%s", str ? str : "", prefix ? (str ? "\n" : "") : " ", value);
            g_free(value);
            g_free(str);

            str = indices;
            indices = g_strdup_printf("%s%s%d", str ? str : "", str ? " " : "", row);
            g_free(str);
...
static void fill_tree_view(CustomElement* el, GList* arglist)
{
    ...
    while (args)
    {
        ...
        if (arg[0] == '^' || !col)
        {
            ...
            {
                str =
                    g_strdup_printf(_("Too many columns (>%d) in %s"), MAX_LIST_COLUMNS, el->name);
            ...
            if (arg[0] == '^')
            {
                ...
                if (sep)
                {
                    sep[0] = ':';
                    sep++;
                    if (!strcmp(sep, "progress") && gtk_tree_view_column_get_visible(col))
            ...
            switch (coltypes[colcount - 1])
            {
                ...
                case G_TYPE_DOUBLE:
                    ...
                    gtk_tree_view_column_set_attributes(col, renderer, "text", colcount - 1, NULL);
    ...
    while (args)
    {
        ...
        if (arg[0] == '^')
        {
            ...
            if (start)
            {
                ...
                valid_iter = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(list), &iter);
            }
        }
...
static void select_in_tree_view(CustomElement* el, const char* value, bool select)
{
    ...
    if (value[0] == '\0')
    {
        if (select)
        {
            ...
            else if (gtk_tree_model_get_iter_first(model, &iter))
            {
                ...
                gtk_tree_view_set_cursor(GTK_TREE_VIEW(view), tree_path, NULL, FALSE);
                gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(view), tree_path, NULL, TRUE, .25, 0);
            }
        }
        else
    ...
    do
    {
        ...
        if (!g_strcmp0(str, value))
        {
            if (!!gtk_tree_selection_iter_is_selected(tree_sel, &iter) != !!select)
            {
                tree_path = gtk_tree_model_get_path(model, &iter);
                if (select)
                {
...
static GList* args_from_file(const char* path)
{
    ...
    if (!file)
    {
        dlg_warn(_("error reading file %s: %s"), path, g_strerror(errno));
        return NULL;
    }
...
static void set_element_value(CustomElement* el, const char* name, char* value)
{
    ...
    switch (el_name->type)
    {
        ...
        case CDLG_LABEL:
            if (el_name->widgets->next && (w = GTK_WIDGET(el_name->widgets->next->data)))
            {
                ...
                if (el_name->val[0] == '~')
                    gtk_label_set_markup_with_mnemonic(GTK_LABEL(w), el_name->val + 1);
                else
            ...
            if (el_name->widgets->next && (w = GTK_WIDGET(el_name->widgets->next->data)))
            {
                ...
                if (!sep &&
                    (!g_strcmp0(el_name->val, "ok") || !g_strcmp0(el_name->val, "cancel") ||
                     ...
                     !g_strcmp0(el_name->val, "yes") || !g_strcmp0(el_name->val, "no") ||
                     !g_strcmp0(el_name->val, "apply") || !g_strcmp0(el_name->val, "delete") ||
                     !g_strcmp0(el_name->val, "edit") || !g_strcmp0(el_name->val, "save") ||
        ...
        case CDLG_PASSWORD:
            ...
            if (el_name->type == CDLG_INPUT_LARGE)
            {
                gtk_text_buffer_set_text(
                    gtk_text_view_get_buffer(GTK_TEXT_VIEW(el_name->widgets->next->data)),
                    str,
                    -1);
            ...
            else
            {
                ...
                gtk_editable_select_region(GTK_EDITABLE(el_name->widgets->next->data), 0, -1);
            }
            g_free(el_name->val);
            el_name->val = str;
            break;
        ...
        case CDLG_EDITOR:
            ...
            {
                // viewer
                buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(el_name->widgets->next->data));
                ...
                if (el_name->option)
                {
                    ...
                    if (gtk_adjustment_get_upper(adj) - gtk_adjustment_get_value(adj) <
                        gtk_adjustment_get_page_size(adj) + 40)
                        gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(el_name->widgets->next->data),
        ...
        case CDLG_RADIO:
            if (el_name->widgets->next)
            {
                if (!strcmp(value, "1") || !strcmp(value, "true"))
                    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(el_name->widgets->next->data),
                                                 TRUE);
                ...
                else
                {
                    // update option label
                    str = unescape(value);
                    gtk_button_set_label(GTK_BUTTON(el_name->widgets->next->data), str);
                    g_free(str);
                }
            }
            break;
        ...
        case CDLG_COMBO:
            if (el_name->widgets->next)
            {
                if (g_file_test(value, G_FILE_TEST_IS_REGULAR))
                {
                    ...
                    gtk_entry_set_text(
                        GTK_ENTRY(gtk_bin_get_child(GTK_BIN(el_name->widgets->next->data))),
                        value);
                else
        ...
        case CDLG_PROGRESS:
            if (el_name->widgets->next)
            {
                ...
                else if (!g_strcmp0(value, "auto-pulse"))
                {
                    if (!el_name->timeout)
                        el_name->timeout =
                            g_timeout_add(200, (GSourceFunc)on_progress_timer, el_name);
                }
                else
                {
                    ...
                    if (!(str && str[0]))
                    {
                        if (i != 0 || (value && value[0] == '0'))
                            ...
                            str = g_strdup(" ");
                        gtk_progress_bar_set_text(GTK_PROGRESS_BAR(el_name->widgets->next->data),
                                                  str);
                        g_free(str);
                    }
                }
        ...
        case CDLG_TIMEOUT:
            if (el_name->widgets->next && el_name->timeout)
            {
                ...
                el_name->timeout = g_timeout_add(1000, (GSourceFunc)on_timeout_timer, el_name);
            }
            break;
        case CDLG_CHOOSER:
            if (el_name->widgets->next)
            {
                i = gtk_file_chooser_get_action(GTK_FILE_CHOOSER(el_name->widgets->next->data));
                if (i == GTK_FILE_CHOOSER_ACTION_SAVE || i == GTK_FILE_CHOOSER_ACTION_CREATE_FOLDER)
                {
                    if (strchr(value, '/'))
                    {
                    ...
                    gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(el_name->widgets->next->data),
                                                  value);
            }
            break;
        default:
    ...
    switch (el_name->type)
    {
        ...
        case CDLG_PASSWORD:
            if (el_name->type == CDLG_INPUT_LARGE)
                ...
                ret = g_strdup(gtk_entry_get_text(GTK_ENTRY(el_name->widgets->next->data)));
            break;
        case CDLG_CHECKBOX:
        case CDLG_RADIO:
            ret = g_strdup(
                gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(el_name->widgets->next->data))
                    ? "1"
                    : "0");
            break;
        case CDLG_DROP:
        ...
        case CDLG_PROGRESS:
            ret =
                g_strdup(gtk_progress_bar_get_text(GTK_PROGRESS_BAR(el_name->widgets->next->data)));
            break;
        case CDLG_WINDOW_SIZE:
            pad = -1;
            ...
            break;
        case CDLG_CHOOSER:
            if (gtk_file_chooser_get_select_multiple(
                    GTK_FILE_CHOOSER(el_name->widgets->next->data)))
            {
                GSList* files =
                    gtk_file_chooser_get_filenames(GTK_FILE_CHOOSER(el_name->widgets->next->data));
                GSList* sl;
                if (files)
                {
        ...
        else
        {
            ...
            else
            {
                if (!strcmp(sep + 1, "v") || !g_strcmp0(sep + 1, el->name))
                    // %v or %self
                    str3 = xvalue ? g_strdup(xvalue) : get_element_value(el, el->name);
                else
                    // %NAME
...
static void internal_command(CustomElement* el, int icmd, GList* args, char* xvalue)
{
    ...
    if (args->next)
    {
        ...
        if (cvalue && (cvalue[0] == '\0' || !strcmp(cvalue, "0") || !strcmp(cvalue, "false")))
            reverse = TRUE;
    }
    if (icmd != CMD_NOOP && icmd != CMD_CLOSE && icmd != CMD_SOURCE && icmd != CMD_FOCUS &&
        icmd != CMD_SHOW && !cname)
    {
        dlg_warn(_("internal command %s requires an argument"), cdlg_cmd[icmd * 3], NULL);
        return;
    }
    ...
    if (icmd == CMD_SET && (!strcmp(cname, "title") || !strcmp(cname, "windowtitle") ||
                            !strcmp(cname, "windowsize") || !strcmp(cname, "windowicon")))
    {
        // generic - no element
        ...
        else if (!strcmp(cname, "windowsize"))
        {
            ...
            if (width > 0 && height > 0)
            {
                ...
                gtk_window_set_position(GTK_WINDOW(el->widgets->data), GTK_WIN_POS_CENTER);
            }
            else
                dlg_warn(_("Dynamic resize requires width and height > 0"), NULL, NULL);
        }
    ...
    switch (icmd)
    {
        ...
        case CMD_PRESS:
            if (el_name->type == CDLG_BUTTON || el_name->type == CDLG_FREE_BUTTON)
                ...
                dlg_warn(_("internal command press is invalid for non-button %s"), cname, NULL);
            break;
        case CMD_SELECT:
        case CMD_UNSELECT:
            ...
            else if (el_name->type == CDLG_DROP)
            {
                if (icmd == CMD_SELECT)
                    ...
                    gtk_combo_box_set_active(GTK_COMBO_BOX(el_name->widgets->next->data), -1);
            }
            else if (el_name->type == CDLG_COMBO)
            {
                if (icmd == CMD_SELECT)
            ...
            {
                if (icmd == CMD_SELECT)
                    gtk_file_chooser_select_filename(GTK_FILE_CHOOSER(el_name->widgets->next->data),
                                                     cvalue);
                else
        ...
        case CMD_SOURCE:
            ...
            if (!out)
            {
                dlg_warn(_("error writing file %s: %s"), cname, g_strerror(errno));
                break;
            }
...
static void run_command_line(CustomElement* el, char* line)
{
    ...
    if (args)
    {
        ...
        if (icmd != -1)
            ...
            dlg_warn(_("'%s' is not an internal command"), (char*)args->data, NULL);
        g_list_foreach(args, (GFunc)g_free, NULL);
        g_list_free(args);
    }
}
...
static void write_file_value(const char* path, const char* val)
{
    ...
    if ((f = open(path + add, O_CREAT | O_WRONLY | O_TRUNC, S_IRUSR | S_IWUSR)) == -1)
    {
        dlg_warn(_("error writing file %s: %s"), path + add, g_strerror(errno));
        return;
    }
    if (val)
    {
...
static char* read_file_value(const char* path, bool multi)
{
    ...
    if (!g_file_test(path, G_FILE_TEST_EXISTS))
    {
        ...
        {
            dlg_warn(_("error creating file %s: %s"), path, g_strerror(errno));
            return NULL;
        }
        close(f);
    }
    ...
    */
        ...
        size > 0)
    {
        if (!g_utf8_validate(line, size, NULL))
            dlg_warn(_("pipe '%s' data is not valid UTF-8"), (char*)el->args->data, NULL);
        else if (el->type == CDLG_VIEWER && el->widgets->next)
        {
            GtkTextIter iter, siter;
            GtkTextBuffer* buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(el->widgets->next->data));
                ...
                gtk_text_buffer_get_line_count(buf) > 800)
            {
                if (gtk_text_buffer_get_char_count(buf) > 64000)
                {
                    ...
                    // trim to 700 lines
                    gtk_text_buffer_get_iter_at_line(buf,
                                                     &iter,
...
static void cb_file_value_change(VFSFileMonitor* fm, VFSFileMonitorEvent event,
                                 const char* file_name, CustomElement* el)
{
    ...
    if (!el->update_timeout)
    {
        ...
        el->update_timeout =
            g_timeout_add_full(G_PRIORITY_DEFAULT_IDLE, 50, (GSourceFunc)delayed_update, el, NULL);
    }
}
...
static void fill_buffer_from_file(CustomElement* el, GtkTextBuffer* buf, char* path, bool watch)
{
    ...
    if (watch && !el->monitor)
    {
        ...
        el->monitor = vfs_file_monitor_add(pathx, FALSE, el->callback, el);
    }
}
...
static void get_text_value(CustomElement* el, const char* val, bool multi, bool watch)
{
    ...
    if (val[0] == '@')
    {
        ...
        if (watch && !el->monitor && g_file_test(val + 1, G_FILE_TEST_IS_REGULAR))
        {
            // start monitoring file
            el->callback = (VFSFileMonitorCallback)cb_file_value_change;
            el->monitor = vfs_file_monitor_add((char*)val + 1, FALSE, el->callback, el);
            el->watch_file = val + 1;
        }
    }
    else
...
static void write_value(FILE* file, const char* prefix, const char* name, const char* sub,
                        const char* val)
{
    char* str;
    ...
    fprintf(file, "%s_%s%s%s=%s\n", prefix, name, sub ? "_" : "", sub ? sub : "", quoted);
    g_free(quoted);
}
...
static void write_source(GtkWidget* dlg, CustomElement* el_pressed, FILE* out, bool is_cancel)
{
            ...
            if (el->args)
            {
                ...
                if (el->val && el->val[0] != '\0')
                {
                    ...
                    if (strcmp(str, el->val))
                        dlg_warn(_("prefix '%s' is not a valid variable name"), el->val, NULL);
                    else
                        prefix = el->val;
                    g_free(str);
                }
    ...
    int button_count = 0;
    fprintf(out,
            "#!%s\n%s\n# SpaceFM Dialog source output - execute this output to set "
            "variables\n# Example: eval \"$(spacefm --dialog --label \"Message\" "
            "--button ok)\"\n\n",
            BASHPATH,
            SHELL_SETTINGS);
    if (!el_pressed)
    ...
    for (l = elements; l; l = l->next)
    {
        ...
        switch (el->type)
        {
            ...
            case CDLG_EDITOR:
                ...
                {
                    // save file
                    write_value(out, prefix, el->name, "saved", (char*)el->args->next->data);
                    GtkTextIter iter, siter;
                    GtkTextBuffer* buf =
                        gtk_text_view_get_buffer(GTK_TEXT_VIEW(el->widgets->next->data));
                    gtk_text_buffer_get_start_iter(buf, &siter);
                    gtk_text_buffer_get_end_iter(buf, &iter);
            ...
            case CDLG_RADIO:
                write_value(out,
                            ...
                            el->name,
                            "label",
                            gtk_button_get_label(GTK_BUTTON(el->widgets->next->data)));
                write_value(out,
                            prefix,
                            el->name,
                            NULL,
                            gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(el->widgets->next->data))
                                ? "1"
                                : "0");
                // save file
                if (!is_cancel && el->args && el->args->next &&
                    ((char*)el->args->next->data)[0] == '@')
                {
                    write_file_value(
                        ...
                        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(el->widgets->next->data))
                            ? "1"
                            : "0");
                    write_value(out, prefix, el->name, "saved", (char*)el->args->next->data + 1);
                }
                break;
            case CDLG_DROP:
            case CDLG_COMBO:
                // write text value
                str =
                    gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(el->widgets->next->data));
                write_value(out, prefix, el->name, NULL, str);
                ...
                str = g_strdup_printf(
                    "%d",
                    gtk_combo_box_get_active(GTK_COMBO_BOX(el->widgets->next->data)));
            ...
            case CDLG_PROGRESS:
                write_value(out,
                            ...
                            el->name,
                            NULL,
                            gtk_progress_bar_get_text(GTK_PROGRESS_BAR(el->widgets->next->data)));
                break;
            case CDLG_WINDOW_SIZE:
                ...
                if (el->args && ((char*)el->args->data)[0] == '@')
                {
                    ...
                    write_value(out, prefix, el->name, "saved", (char*)el->args->data + 1);
                    g_free(str);
                }
                break;
            case CDLG_CHOOSER:
                if (gtk_file_chooser_get_select_multiple(GTK_FILE_CHOOSER(el->widgets->next->data)))
                {
                    GSList* files =
                        gtk_file_chooser_get_filenames(GTK_FILE_CHOOSER(el->widgets->next->data));
                    GSList* sl;
                    if (!files)
                        fprintf(out, "%s_%s=''\n", prefix, el->name);
                    else
                ...
                {
                    str = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(el->widgets->next->data));
                    write_value(out, prefix, el->name, NULL, str);
                    g_free(str);
                }
                str =
                    gtk_file_chooser_get_current_folder(GTK_FILE_CHOOSER(el->widgets->next->data));
...
static void on_widget_grab_focus(GtkWidget* widget, CustomElement* el)
{
    ...
    for (l = elements; l; l = l->next)
    {
        if (((CustomElement*)l->data)->type == CDLG_CLICK && ((CustomElement*)l->data)->cmd_args)
            run_command((CustomElement*)l->data, ((CustomElement*)l->data)->cmd_args, val);
    }
    g_free(val);
    g_free(elval);
}
...
static void on_list_row_activated(GtkTreeView* view, GtkTreePath* tree_path, GtkTreeViewColumn* col,
                                  CustomElement* el)
{
    GtkTreeIter iter;
...
static void on_option_toggled(GtkToggleButton* togglebutton, CustomElement* el)
{
    if (el->type == CDLG_TIMEOUT)
    {
        if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(el->widgets->next->data)))
        {
            if (el->timeout)
            {
    ...
    else if (el->cmd_args)
    {
        ...
        {
            run_command(el, el->cmd_args, gtk_toggle_button_get_active(togglebutton) ? "1" : "0");
        }
    }
}
...
static void on_button_clicked(GtkButton* button, CustomElement* el)
{
    ...
    {
        // no command
        write_source(el->widgets->data, el, stdout, !g_strcmp0(el->val, "cancel"));
        g_idle_add((GSourceFunc)destroy_dlg, el->widgets->data);
    }
}
...
static bool on_dlg_key_press(GtkWidget* entry, GdkEventKey* evt, CustomElement* el)
{
    ...
    {
        char* str = g_strdup_printf("%s %s", (char*)el->args->data, (char*)el->args->next->data);
        run_command(el, el->cmd_args, str);
        g_free(str);
        return TRUE;
    }
...
static bool on_input_key_press(GtkWidget* entry, GdkEventKey* evt, CustomElement* el)
{
    ...
    if ((el->type == CDLG_INPUT || el->type == CDLG_INPUT_LARGE) && el->cmd_args)
    {
        // input has a command
        if (el->type == CDLG_INPUT_LARGE)
            run_command(el, el->cmd_args, NULL);
    ...
    while (args && g_str_has_prefix((char*)args->data, "--"))
    {
        if (!strcmp((char*)args->data, "--compact"))
            ...
            expand = TRUE;
                 ...
                 !strcmp((char*)args->data, "--select") && args->next)
        {
            args = args->next;
        ...
        else if (el->type == CDLG_CHOOSER)
        {
            ...
            else if (!strcmp((char*)args->data, "--filter"))
            {
                ...
                {
                    args = args->next;
                    chooser_filters = g_list_append(chooser_filters, (char*)args->data);
                }
            }
        }
        args = args->next;
    ...
    switch (el->type)
    {
        ...
        case CDLG_FREE_BUTTON:
            ...
            if (!el->widgets->next)
            {
                ...
                if (el->type == CDLG_BUTTON)
                {
                    ...
                    gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(w), expand, TRUE, pad);
                g_signal_connect(G_OBJECT(w), "clicked", G_CALLBACK(on_button_clicked), el);
                if (radio)
                    *radio = NULL;
                if (args)
                    el->cmd_args = el->args->next;
            ...
            if (el->widgets->next && (w = GTK_WIDGET(el->widgets->next->data)))
            {
                ...
                if (!sep && (!g_strcmp0(el->val, "ok") || !g_strcmp0(el->val, "cancel") ||
                             ...
                             !g_strcmp0(el->val, "apply") || !g_strcmp0(el->val, "delete") ||
                             !g_strcmp0(el->val, "edit") || !g_strcmp0(el->val, "save") ||
        ...
        case CDLG_IMAGE:
            ...
            if (!el->widgets->next && box)
            {
                ...
                gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(image_box), expand, TRUE, pad);
                g_signal_connect(G_OBJECT(image_box),
                                 "button-press-event",
            ...
            else if (el->widgets->next && args && ((char*)args->data)[0] == '@')
            {
                ...
                if (g_strcmp0(str, el->val))
                {
                    ...
                    else
                    {
                        ...
                        gtk_editable_select_region(GTK_EDITABLE(el->widgets->next->data), 0, -1);
                    }
                }
                g_free(str);
            }
        ...
        case CDLG_EDITOR:
                ...
                el->widgets->next)
            {
                ...
                if (el->option)
                {
                    ...
                    GtkAdjustment* adj = gtk_scrolled_window_get_vadjustment(
                        GTK_SCROLLED_WINDOW(el->widgets->next->next->data));
                    if (selstart || gtk_adjustment_get_upper(adj) - gtk_adjustment_get_value(adj) <
                                        gtk_adjustment_get_page_size(adj) + 40)
                        gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(el->widgets->next->data),
                                                     gtk_text_buffer_get_mark(buf, "end"),
                                                     0.0,
        ...
        case CDLG_COMMAND:
            if (!el->option && args)
            {
                if (((char*)args->data)[0] != '\0' && stat((char*)args->data, &statbuf) != -1 &&
                    S_ISFIFO(statbuf.st_mode))
                {
                    // watch pipe
                    GIOChannel* channel = g_io_channel_new_file((char*)args->data, "r+", NULL);
        ...
        case CDLG_RADIO:
            ...
            if (!el->widgets->next && box && radio)
            {
                ...
                g_signal_connect(G_OBJECT(w), "toggled", G_CALLBACK(on_option_toggled), el);
            }
            else if (el->widgets->next && args && args->next)
            {
                get_text_value(el, (char*)args->next->data, FALSE, TRUE);
                if (!g_strcmp0(el->val, "1") || !g_strcmp0(el->val, "true"))
                    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(el->widgets->next->data), TRUE);
                else if (!g_strcmp0(el->val, "0") || !g_strcmp0(el->val, "false"))
                    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(el->widgets->next->data), FALSE);
            }
            break;
        case CDLG_DROP:
        case CDLG_COMBO:
            ...
            if (!el->widgets->next && box)
            {
                ...
                else
                {
                    ...
                    g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN(w))),
                                     "grab-focus",
                                     G_CALLBACK(on_widget_grab_focus),
                                     el);
                }
            ...
            if (el->widgets->next && args)
            {
                if (((char*)args->data)[0] == '@')
                {
                    ...
                    if (!el->monitor && args)
                    {
                        // start monitoring file
                        el->callback = (VFSFileMonitorCallback)cb_file_value_change;
                        el->monitor = vfs_file_monitor_add(str, FALSE, el->callback, el);
                    }
                    // free temp args
                    g_list_foreach(args, (GFunc)g_free, NULL);
                    g_list_free(args);
        ...
        case CDLG_PROGRESS:
            if (!el->widgets->next && box)
            {
                                       ...
                                       !strcmp((char*)args->data, "auto-pulse"))))
                {
                    ...
                    gtk_progress_bar_set_text(GTK_PROGRESS_BAR(el->widgets->next->data), " ");
                ...
                else
                {
                    ...
                    if (!(str && str[0]))
                    {
                        if (i != 0 || (el->val && el->val[0] == '0'))
                            ...
                            str = g_strdup(" ");
                        gtk_progress_bar_set_text(GTK_PROGRESS_BAR(el->widgets->next->data), str);
                        g_free(str);
                    }
                }
            }
        ...
        case CDLG_CHOOSER:
            if (!el->widgets->next && box)
            {
                ...
                g_signal_connect(G_OBJECT(w),
                                 "file-activated",
                                 G_CALLBACK(on_chooser_activated),
                                 el);
                g_signal_connect(G_OBJECT(w),
                                 "selection-changed",
            ...
            if (args && el->widgets->next)
            {
                ...
                if (el->val)
                {
                    if (chooser_save && (chooser_dir || !g_file_test(el->val, G_FILE_TEST_IS_DIR)))
                    {
                        if (strchr(el->val, '/'))
                        {
                        ...
                        gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(el->widgets->next->data),
                                                      el->val);
                }
            }
            break;
        case CDLG_KEYPRESS:
            if (!el->cmd_args && args && args->next && args->next->next)
            {
                ...
                if (keycode != 0)
                    g_signal_connect(G_OBJECT(el->widgets->data),
                                     "key-press-event",
                                     G_CALLBACK(on_dlg_key_press),
                                     el);
                el->cmd_args = args->next->next;
                if (radio)
                    *radio = NULL;
...
static void build_dialog(GList* elements)
{
    ...
    for (l = elements; l; l = l->next)
    {
        ...
        if (!focus_el &&
            (el->type == CDLG_INPUT || el->type == CDLG_INPUT_LARGE || el->type == CDLG_EDITOR ||
             el->type == CDLG_COMBO || el->type == CDLG_PASSWORD))
            focus_el = el;
        else if ((el->type == CDLG_HBOX || el->type == CDLG_VBOX) && el->widgets->next)
        {
        ...
        else if (el->type == CDLG_WINDOW_CLOSE && el->args)
        {
            ...
            {
                g_signal_connect(G_OBJECT(dlg), "delete-event", G_CALLBACK(on_window_delete), el);
                has_delete_handler = TRUE;
            }
        }
        else if (el->type == CDLG_TIMEOUT && el->option && !el->widgets->next && !timeout_added)
        {
            el->widgets = g_list_append(el->widgets, timeout_toggle);
            el->timeout = g_timeout_add(1000, (GSourceFunc)on_timeout_timer, el);
            g_signal_connect(G_OBJECT(timeout_toggle),
                             "toggled",
                             G_CALLBACK(on_option_toggled),
                             el);
            g_free(el->val);
            el->val = g_strdup_printf("%s %d", _("Pause"), el->option);
            gtk_button_set_label(GTK_BUTTON(el->widgets->next->data), el->val);
        ...
        if (!is_large && el->widgets->next &&
            (el->type == CDLG_CHOOSER || el->type == CDLG_MLIST || el->type == CDLG_EDITOR ||
             el->type == CDLG_VIEWER || el->type == CDLG_LIST))
            is_large = TRUE;
    }
    g_list_free(boxes);
    ...
    if (!has_delete_handler)
        g_signal_connect(G_OBJECT(dlg), "delete-event", G_CALLBACK(on_window_delete), NULL);

    // resize window
    if (is_large && !is_sized)
        gtk_window_set_default_size(GTK_WINDOW(dlg), DEFAULT_LARGE_WIDTH, DEFAULT_LARGE_HEIGHT);
    ...
    if (focus_el && focus_el->widgets->next)
    {
        ...
        if (focus_el->type == CDLG_INPUT && focus_el->option >= 0)
        {
            ...
            gtk_editable_select_region(GTK_EDITABLE(focus_el->widgets->next->data),
                                       focus_el->option,
                                       focus_el->option2);
        }
    }
    ...
    for (l = elements; l; l = l->next)
    {
        if (((CustomElement*)l->data)->type == CDLG_COMMAND && ((CustomElement*)l->data)->cmd_args)
            run_command((CustomElement*)l->data, ((CustomElement*)l->data)->cmd_args, NULL);
    }
